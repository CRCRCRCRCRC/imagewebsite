...
export async function renameImage(imageId, name) {
  ...
  await updateImageNames((imageNames) => {
...
export async function deleteImage(imageId) {
  ...
  try {
    ...
    // The image file is already gone. Metadata cleanup should not make delete look failed.
    console.error("Image name metadata cleanup failed after delete.", error);
  }
}
...
export async function loadNotebookState() {
  const privateResult = await get(NOTEBOOK_STATE_PATH, {
    access: PRIVATE_METADATA_ACCESS,
    useCache: false,
  });

  if (privateResult?.statusCode === 200) {
    try {
      const text = await new Response(privateResult.stream).text();
      return normalizeNotebookState(JSON.parse(text));
    } catch (error) {
      console.error("Notebook state parsing failed.", error);
      return createDefaultNotebookState();
    }
  }

  const publicResult = await get(NOTEBOOK_STATE_PATH, {
  ...
  try {
    const text = await new Response(publicResult.stream).text();
    return normalizeNotebookState(JSON.parse(text));
  } catch (error) {
    console.error("Notebook state parsing failed.", error);
    return createDefaultNotebookState();
...
export async function saveNotebookState(state) {
  const normalizedState = normalizeNotebookState(state, { withTimestamp: true });

  await put(NOTEBOOK_STATE_PATH, JSON.stringify(normalizedState), {
    access: PRIVATE_METADATA_ACCESS,
    addRandomSuffix: false,
    allowOverwrite: true,
...
export async function uploadImage(file, folderId) {
  ...
  const originalUploadPromise = put(pathname, file, {
    access: "public",
    addRandomSuffix: false,
    contentType: file.type || "application/octet-stream",
  });
  const thumbnailUploadPromise = createThumbnailBuffer(fileBuffer)
    .then((thumbnailBuffer) =>
      put(thumbnailPath(folderId, imageToken, safeFileName), thumbnailBuffer, {
        access: "public",
        addRandomSuffix: false,
        contentType: "image/webp",
      }),
  ...
  const [blob, thumbnailBlob] = await Promise.all([originalUploadPromise, thumbnailUploadPromise]);
  const uploadedAt = blob.uploadedAt || uploadStartedAt;

  return {
    id: blob.pathname,
    folderId,
    folderName: decodeFolderId(folderId),
    name: getDisplayImageName(safeFileName),
    originalName: safeFileName,
    size: blob.size || file.size,
    uploadedAt,
    url: withBlobVersion(blob.url, uploadedAt),
    thumbnailUrl: withBlobVersion(thumbnailBlob?.url || blob.url, thumbnailBlob?.uploadedAt || uploadedAt),
  };
}
...
async function loadImageNames() {
  const snapshot = await loadImageNamesSnapshot();
  return snapshot.names;
}

async function saveImageNames(imageNames, { ifMatch } = {}) {
...
async function parseImageNames(result) {
  try {
...
async function loadImageNamesSnapshot() {
  const [privateState, publicState] = await Promise.all([
    readImageNamesBlob(PRIVATE_METADATA_ACCESS, { useCache: false }),
    readImageNamesBlob(METADATA_ACCESS),
  ]);
  ...
  return {
    ...
    etag: privateState.etag,
  };
}

async function readImageNamesBlob(access, extraOptions = {}) {
  try {
    ...
    return {
      names: await parseImageNames(result),
      etag: result.blob?.etag || null,
    };
  } catch (error) {
    console.error(`Image name metadata read failed (${access}).`, error);
    return {
      names: {},
      etag: null,
    };
...
async function updateImageNames(mutator) {
  for (let attempt = 0; attempt < IMAGE_NAME_WRITE_RETRIES; attempt += 1) {
    const snapshot = await loadImageNamesSnapshot();
    const nextImageNames = { ...snapshot.names };
    mutator(nextImageNames);

    try {
      await saveImageNames(nextImageNames, {
        ifMatch: snapshot.etag || undefined,
      });
      return nextImageNames;
    } catch (error) {
      if (isBlobPreconditionFailure(error) && attempt < IMAGE_NAME_WRITE_RETRIES - 1) {
        continue;
      }
  ...
  throw new Error("圖片名稱同步衝突，請再試一次。");
}

function isBlobPreconditionFailure(error) {
  return error?.name === "BlobPreconditionFailedError" || error?.status === 412;
}
...
function normalizeNotebookState(state, { withTimestamp = false } = {}) {
  const baseState = state && typeof state === "object" ? state : {};
  const content = typeof baseState.content === "string" ? baseState.content : "";
  const todos = Array.isArray(baseState.todos)
    ? baseState.todos.map(normalizeNotebookTodo).filter(Boolean)
    : [];

  return {
    content,
    todos,
    updatedAt: withTimestamp ? new Date().toISOString() : baseState.updatedAt || null,
  };
}

function normalizeNotebookTodo(todo) {
  ...
  const textSource = todo.text ?? todo.label ?? todo.title ?? todo.content ?? "";
  ...
  return {
    ...
    text,
    price: normalizeTodoValue(todo.price ?? todo.amount ?? todo.value ?? ""),
    done: Boolean(todo.done ?? todo.checked ?? todo.completed),
  };
}
