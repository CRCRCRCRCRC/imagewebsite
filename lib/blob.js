...
export async function listLibrary() {
  ...
  const thumbnailUrlByImageId = new Map(
    imageResult.blobs
      .filter((blob) => isThumbnailPath(blob.pathname))
      .map((blob) => [getImageToken(blob.pathname), blob.url]),
  );
  ...
  const images = imageResult.blobs
    ...
    .map((blob) => {
      ...
      return {
        ...
        url: blob.url,
        thumbnailUrl: thumbnailUrlByImageId.get(getImageToken(blob.pathname)) || blob.url,
      };
    })
...
export async function renameImage(imageId, name) {
  ...
  const targetImage = library.images.find((image) => image.id === imageId);
  if (!targetImage) {
