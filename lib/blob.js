...
async function loadImageNames() {
  const privateResult = await get(IMAGE_NAME_METADATA_PATH, {
    access: PRIVATE_METADATA_ACCESS,
    useCache: false,
  });

  if (privateResult?.statusCode === 200) {
    return parseImageNames(privateResult);
  }

  const publicResult = await get(IMAGE_NAME_METADATA_PATH, {
    access: METADATA_ACCESS,
  });

  if (publicResult?.statusCode === 200) {
    return parseImageNames(publicResult);
  }
